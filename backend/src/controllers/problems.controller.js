...
import { getJudge0LanguageId, poolBatchResults } from "../libs/judge0.libs.js"



export const createProblem = async (req, res) => {
    //going to get the all data from request body
    const { title, description, difficulty, tags, examples, constrains, hints, testcases, codeSnippets, referenceSolutions } = req.body
    ...
    try {
        ...
        const newProblem = await db.problem.create({
            data: {
                title, description, difficulty, tags, examples, constrains, hints, testcases, codeSnippets, referenceSolutions, userId: req.user.id
            }
        })
